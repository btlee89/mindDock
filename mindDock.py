...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QTimer, QUrl, QSettings, QPoint
...
class MyWebEnginePage(QWebEnginePage):
    ...
    def featurePermissionRequested(self, url, feature):
        ...
        if feature == QWebEnginePage.Feature.Clipboard: self.setFeaturePermission(url, feature, QWebEnginePage.PermissionPolicy.PermissionGrantedByUser)
        else:
    ...
    def createWindow(self, windowType):
        """
        웹 페이지가 새 창(window.open, target="_blank" 등)을 열려고 할 때 호출됩니다.
        이 요청을 가로채서 기본 시스템 브라우저에서 URL을 열도록 합니다.
        """
        # createWindow가 호출되면, QWebEnginePage는 새로운 페이지 객체를 생성하려고 합니다.
        # 여기서 새로운 QWebEnginePage를 만들고, 그 페이지의 urlChanged 시그널을 연결하여
        # URL이 설정되는 순간을 포착합니다.
        new_page = MyWebEnginePage(self.profile(), self)

        @new_page.urlChanged.connect
        def on_url_changed(url):
            # URL이 변경되면(즉, 새 창의 목적지 주소가 설정되면)
            # 연결된 슬롯을 끊고, 해당 URL을 기본 브라우저에서 엽니다.
            new_page.urlChanged.disconnect()
...
class OllamaChatApp(QMainWindow):
    ...
    def __init__(self):
        ...
        if sys.platform == "win32":
            ...
            if local_appdata:
                ...
                self.log_path = os.path.join(os.path.expanduser("~"), self.APP_NAME, "logs")
        else:
            # Linux/macOS: ~/.cache/mindDock/logs 경로 사용
            self.log_path = os.path.join(os.path.expanduser("~"), ".cache", self.APP_NAME, "logs")
        ...
        # 🌟 FIX: ChatGPT, Perplexity 세션 유지를 위한 타이머 추가
        self.session_keep_alive_timers = {}
        self.SESSION_KEEP_ALIVE_INTERVAL = 600000 # 10분
        ...
        self.webview_load_status = {api: False for api in ["Gemini", "Perplexity", "Copilot", "ChatGPT"]} # 🌟 FIX: 웹뷰 로드 완료 상태 추적
        ...
        settings_path = os.path.join(self.base_profile_path, f"{self.APP_NAME}.conf")
        ...
        # 🌟 FIX: 각 웹뷰의 loadFinished 시그널에 로드 상태를 업데이트하는 람다 함수 연결
        for api_name, view in self.web_views.items():
            view.loadFinished.connect(lambda ok, name=api_name: self.on_webview_load_finished(ok, name))
        ...
        # 🌟 FIX: ChatGPT, Perplexity를 위한 세션 유지 타이머 초기화
        for api_name in ["ChatGPT", "Perplexity"]:
            view = self.web_views.get(api_name)
            if view:
                timer = QTimer(self)
                # 🌟 FIX: reload를 직접 호출하는 대신, 로드 상태를 확인하는 함수를 호출하도록 변경
                timer.timeout.connect(lambda name=api_name: self.keep_session_alive(name))
                self.session_keep_alive_timers[api_name] = timer
    ...
    # 🌟 FIX: 웹뷰 로드 완료 시 상태를 기록하는 함수
    def on_webview_load_finished(self, ok, api_name):
        """웹뷰 로드가 완료되면 상태 플래그를 True로 설정합니다."""
        if ok:
            self.webview_load_status[api_name] = True
    ...
    def _create_readme_if_not_exists(self):
        ...
        if not os.path.exists(readme_path):
            # 🌟 FIX: 클래스 상수를 사용하여 Readme 내용 생성
            readme_content = f"""==================================================
{self.APP_NAME} - 통합 AI Chat Hub
==================================================

{self.APP_NAME}은 Ollama 로컬 모델과 주요 웹 AI 서비스를 하나의 인터페이스에서 사용할 수 있도록 만든 통합 AI 채팅 허브입니다.
...
- **상세한 환경설정**: `{self.APP_NAME}.conf` 파일을 통해 창 크기, 캐시 정책, 서버 주소 등을 직접 설정할 수 있습니다.
- **독립된 인증 창**: PIN, Passkey 등 복잡한 인증 방식도 별도의 창을 통해 안전하게 처리합니다.

---
### 환경설정 ({self.APP_NAME}.conf 파일)

앱을 한 번 실행하면 아래 위치에 설정 파일이 생성됩니다. 이 파일을 직접 수정하여 앱의 동작을 변경할 수 있습니다.

- **파일 위치**:
  - Linux/macOS: `~/.config/{self.APP_NAME}_profiles/{self.APP_NAME}.conf`
  - Windows: `%APPDATA%\\{self.APP_NAME}\\profiles\\{self.APP_NAME}.conf`
...
- **프로필 데이터 (로그인 정보, 쿠키 등)**:
  - Linux/macOS: `~/.config/{self.APP_NAME}_profiles/`
  - Windows: `%APPDATA%\\{self.APP_NAME}\\profiles\\`
  - 각 웹뷰의 데이터는 이 폴더 아래에 서비스 이름(Gemini, Copilot 등)으로 된 하위 폴더에 개별적으로 저장됩니다.

- **대화 로그**:
  - Linux/macOS: `~/.cache/{self.APP_NAME}/logs/`
  - Windows: `%LOCALAPPDATA%\\{self.APP_NAME}\\logs\\`
...
"""
    ...
    def _save_window_state(self):
        ...
        if screen_geom.width() > 0 and screen_geom.height() > 0:
            ...
            self.settings.setValue("window/x_ratio", x_ratio)
            self.settings.setValue("window/y_ratio", y_ratio)
            self.settings.setValue("window/w_ratio", w_ratio)
            self.settings.setValue("window/h_ratio", h_ratio)
    ...
    def _load_window_state(self):
        """QSettings에서 저장된 창 상태를 복원합니다."""
        if not self.settings.value("save_window_state", True, type=bool):
            return

        # 저장된 비율 값들 읽기
        x_ratio = self.settings.value("window/x_ratio", 0.1, type=float)
        y_ratio = self.settings.value("window/y_ratio", 0.1, type=float)
        w_ratio = self.settings.value("window/w_ratio", 0.8, type=float)
        h_ratio = self.settings.value("window/h_ratio", 0.8, type=float)
        ...
        # 비율을 절대 좌표로 변환
        x = int(screen_rect.x() + screen_rect.width() * x_ratio)
        y = int(screen_rect.y() + screen_rect.height() * y_ratio)
    ...
    def set_window_icon(self):
        ...
        if sys.platform == "win32":
            ...
            icon_path = os.path.join(base_path, f"{self.APP_NAME}.png")
        else:
            # Linux: 표준 아이콘 경로 사용
    ...
    def update_title(self):
        ...
        if self.current_api == "Ollama":
            title = f'{self.APP_NAME} - 통합 AI Chat Hub [Ollama(Local) - {self.current_model}]'
        else:
            title = f'{self.APP_NAME} - 통합 AI Chat Hub [{self.current_api}]'
    ...
    def show_about_dialog(self):
        ...
        about_dialog.setWindowTitle(f"About {self.APP_NAME}")
        ...
        # 🌟 FIX: 클래스 상수를 사용하여 다이얼로그 내용 생성
        title = f"<h3>{self.APP_NAME} {self.APP_VERSION}</h3>"
        description = f"<p>Ollama 및 주요 웹 AI 서비스를 통합하여<br>하나의 인터페이스에서 사용할 수 있는 AI Chat Hub입니다.</p>"
        contact = f"<p><b>Contact:</b> {self.CONTACT_EMAIL}</p>"
        copyright_info = f"<p>Copyright © {self.COPYRIGHT_YEAR} {self.AUTHOR}. All rights reserved.</p>"
    ...
    # 🌟 FIX: 세션 유지를 위해 웹뷰를 reload하는 함수 (로드 완료 확인)
    def keep_session_alive(self, api_name):
        """웹뷰의 로드가 완료된 상태라면, 세션 유지를 위해 reload를 실행합니다."""
        if self.webview_load_status.get(api_name):
            view = self.web_views.get(api_name)
            if view:
                view.reload()
                print(f"INFO: [{api_name}] 세션 유지를 위해 페이지를 새로고침했습니다.")
        else:
            print(f"INFO: [{api_name}] 페이지가 아직 로드되지 않아 세션 유지를 건너뜁니다.")
    # 🌟 추가: 슬립 탭 관련 함수들
        ...
        for api_name, timer in self.sleep_timers.items():
            if api_name == self.current_api:
                ...
                if self.sleep_state[api_name] == "awake":
                    ...
                    # 🌟 FIX: ChatGPT, Perplexity는 깊은 절전 대신 세션 유지 타이머를 사용
                    if api_name not in self.session_keep_alive_timers:
                        timer.start(self.SLEEP_TIMEOUT)

        # 🌟 FIX: 세션 유지 타이머 관리
        for api_name, timer in self.session_keep_alive_timers.items():
            if api_name == self.current_api:
                # 활성 탭이 되면 세션 유지 타이머 중지
                if timer.isActive():
                    timer.stop()
                    print(f"INFO: [{api_name}] 탭이 활성화되어 세션 유지 타이머를 중지합니다.")
            else:
                # 🌟 FIX: 비활성 탭이 되면 세션 유지 타이머를 즉시 시작하도록 수정
                # 기존에는 첫 인터벌 후에 시작되어, 앱 재시작 직후에는 동작하지 않는 문제가 있었습니다.
                if not timer.isActive():
                    timer.start(self.SESSION_KEEP_ALIVE_INTERVAL)
                    print(f"INFO: [{api_name}] 탭이 비활성화되어 세션 유지를 시작합니다. ({self.SESSION_KEEP_ALIVE_INTERVAL // 60000}분 간격)")
                    QTimer.singleShot(0, timer.timeout.emit) # 즉시 1회 실행하여 세션을 활성화
    ...
    def refresh_current_view(self):
        """
        현재 활성화된 웹뷰를 새로고침합니다.
        단순 reload가 아닌, about:blank를 거쳐 리소스를 초기화한 후 다시 로드하여
        웹페이지 자체의 메모리 누수 등으로 인한 성능 저하 문제를 해결합니다.
        """
        if self.current_api != "Ollama":
            current_web_view = self.web_views.get(self.current_api)
            if current_web_view:
                # 🌟 FIX: about:blank를 거쳐 리소스를 초기화하는 방식으로 새로고침 강화
                url_map = {
                    "Gemini": "https://gemini.google.com/app",
                    "Copilot": "https://copilot.microsoft.com/",
                    "ChatGPT": "https://chat.openai.com/",
                ...
                restore_url = url_map.get(self.current_api)
                if restore_url:
                    current_web_view.setUrl(QUrl(restore_url))
                    self._update_status_indicator("complete", f"{self.current_api} 탭을 새로고침했습니다.")
                    QTimer.singleShot(2000, self._restore_current_status)
...
if __name__ == '__main__':
    ...
    if sys.platform == "win32":
        if getattr(sys, 'frozen', False):
            ...
            base_path = os.path.dirname(os.path.abspath(__file__))
        icon_path = os.path.join(base_path, f"{OllamaChatApp.APP_NAME}.png")
    else:
        # Linux: 표준 아이콘 경로 사용
        icon_path = os.path.expanduser("~/.local/share/icons/mindDock.png")
